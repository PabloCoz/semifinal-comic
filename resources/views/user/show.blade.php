            ...
            <div class="grid grid-cols-1 md:grid-cols-5 lg:grid-cols-7 gap-5">
                <div class="col-span-1 md:col-span-2 -mt-12 relative z-10">
                    <div class="flex justify-center">
                        <img class="h-52 w-52 rounded-full object-cover object-center" src="{{ $user->profile_photo_url }}" alt="" loading="lazy">
                    </div>
                    <div class="my-5">
                    ...
                    <div class="flex justify-center mt-5">
                        <div class="w-full">
                            ...
                            <form action="{{ route('users.desubscription', $user) }}" method="POST">
                                @csrf
                                <button type="submit" class="p-3 bg-red-400 text-white hover:bg-white hover:text-black rounded-full font-bold w-full">
                                    Dejar de seguir
                                </button>
                            </form>
                            @else
                            ...
                            <form action="{{ route('users.original', $user) }}" method="POST">
                                @csrf
                                <button type="submit" class="p-3 bg-red-400 rounded-full font-bold w-full text-white">
                    ...
                    <article class="mt-8">
                        @if($user->profile->facebook != null || $user->profile->instagram != null)
                        ...
                        <div class="mt-5 space-x-3 flex justify-center items-center">
                            ...
                            <a href="{{ $user->profile->facebook }}" class="bg-blue-600 px-3 py-2 text-white font-bold font-josefin text-xl rounded-full">Facebook</a>
                            @endif
                            @if ($user->profile->instagram != null)
                            <a href="{{ $user->profile->instagram }}" class="bg-gradient-to-r from-pink-600 via-red-500 to-yellow-500 px-3 py-2 text-white font-bold font-josefin text-xl rounded-full">Instagram</a>
                            @endif
                        </div>
                        @endif
                    </article>
                    <article class="mt-8">
                        ...
                        <div>
                            ...
                            <ul class="mt-5">
                                ...
                                <a href="{{ route('comics.show', $user->comics_created->first()) }}">
                                    <div>
                                        <div class="flex justify-center items-center">
                                            <img src="{{ Storage::url($user->comics_created->first()->img) }}" class="w-full h-64 object-cover object-center rounded-lg" loading="lazy">
                                        </div>
                                        <div class="mt-2">
                                            <h1 class="text-white font-bold font-josefin text-lg text-center uppercase">
                                ...
                                <div>
                                    <div class="flex justify-center items-center">
                                        <img src="{{ Storage::url($user->comics_created->first()->img) }}" class="w-44 h-64 object-cover object-center rounded-lg" loading="lazy">
                                    </div>
                                    <div class="mt-2">
                                        <h1 class="text-white font-bold font-josefin text-lg text-center uppercase">
                                            {{ $user->comics_created->first()->title }}
